...
'''
@Author: Zhao YinFa
@Time: 2018.3.21 14:34
@Function:测试tensorflow
'''
...
#
# ##构造数据##
# x_data=np.random.rand(100).astype(np.int32) #随机生成100个类型为float32的值
# #x_data=np.random.rand(100).astype(np.int32)
# y_data=x_data*5+3  #定义方程式y=x_data*A+B
#
#
# ##建立TensorFlow神经计算结构##
...
# sess=tf.Session(config=tf.ConfigProto(log_device_placement=True))  #建立TensorFlow训练会话
# sess.run(init)     #将训练结构装载到会话中
#
# start = time.time()
# for step in range(400): #循环训练400次
#      sess.run(train)  #使用训练器根据训练结构进行训练
#      if  step%50==0:  #每20次打印一次训练结果
#
#         print(step,sess.run(weight),sess.run(biases)) #训练次数，A值，B值
#
# end = time.time()
# print("total time is ",end-start,"secs.")
