...
#     if not name:
#         tkinter.messagebox.showinfo('温馨提示', '请输入名字后再继续')
#         return
...
def download():
    ...
    result = r.text
    # print (result)
    reg = re.compile('<img src="..(.*?).gif')
    img_url = start_url + re.findall(reg, result)[0] + '.gif'
    # print (img_url)
    response_r = requests.get(img_url)
    print(response_r.status_code)
    response = response_r.content
    # 将生成的签名图片下载到本地
